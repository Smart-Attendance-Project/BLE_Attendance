...
export default function Export() {
  ...
  return (
    <div className="p-6 max-w-xl mx-auto">
      ...
      <div className="bg-zinc-50 border-2 border-black rounded-xl p-6 shadow-[6px_6px_0_0_#000] flex flex-col gap-5">
        <div>
          <label className="block text-sm font-bold text-zinc-700 mb-1.5">Class / Subject</label>
          <select value={assignmentId} onChange={e => setAssignmentId(e.target.value)}
            className="w-full border-2 border-black rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-400 shadow-[2px_2px_0_0_#000]">
          ...
          <div className="flex gap-2">
            {PRESETS.map(p => (
              <button key={p.label} onClick={() => { setFromDate(p.from); setToDate(p.to) }}
                className="px-3 py-1.5 rounded-lg text-sm font-bold border-2 border-black bg-white hover:bg-indigo-100 transition-colors shadow-[2px_2px_0_0_#000]">
                {p.label}
              </button>
            ))}
          </div>
        ...
        <button onClick={doExport} disabled={loading}
          className="flex items-center justify-center gap-2 bg-indigo-100 hover:bg-indigo-100 disabled:opacity-50 text-black font-black py-3 rounded-xl border-2 border-black shadow-[4px_4px_0_0_#000] hover:shadow-[2px_2px_0_0_#000] hover:translate-x-0.5 hover:translate-y-0.5 transition-all">
